    ...
    class FileUploader
    {
        ...
        function handleUpload($replaceOldFile = FALSE)
        {
            ...
            if (! is_writable($uploadPath)) {
                return array(
                    'error' => __('Server error. Upload directory is not writable.', $bkpkFM->name)
                );
            }
            
            if (! $this->file) {
                return array(
                    'error' => __('No files were uploaded.', $bkpkFM->name)
                );
            }
            ...
            if ($size == 0) {
                return array(
                    'error' => __('File is empty', $bkpkFM->name)
                );
            }
            
            if ($size > $this->sizeLimit) {
                return array(
                    'error' => __('File is too large', $bkpkFM->name)
                );
            }
            ...
            if ($this->allowedExtensions && ! in_array(strtolower($ext), $this->allowedExtensions)) {
                ...
                return array(
                    'error' => sprintf(__('File %1$s has an invalid extension, it should be one of %2$s.', $bkpkFM->name), $pathinfo['filename'], $these)
                );
            }
            ...
            if ($this->file->save($uploadPath . $filename . '.' . $ext)) {
                ...
                return array(
                    'error' => __('Uploaded file could not be saved.', $bkpkFM->name) . __('The upload was cancelled due to server error', $bkpkFM->name)
                );
            }
        }
    }
