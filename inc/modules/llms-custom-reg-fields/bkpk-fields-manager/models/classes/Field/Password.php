...
class Password extends Multiple
{
    ...
    private function getCurrentPasswordInstance()
    {
        ...
        if (! empty($this->label)) {
            $instance->label = isset($this->field['current_pass_title']) ? $this->field['current_pass_title'] : sprintf(__("Current %s", $bkpkFM->name), $this->label);
        }
        if (! empty($this->placeholder)) {
            $instance->placeholder = sprintf(__('Current %s', $bkpkFM->name), $this->placeholder);
    ...
    private function getNewPasswordInstance()
    {
        ...
        if ($this->requiredCurrent) {
            if (! empty($this->label)) {
                $instance->label = isset($this->field['new_pass_title']) ? $this->field['new_pass_title'] : sprintf(__("New %s", $bkpkFM->name), $this->label);
            }
            if (! empty($this->placeholder)) {
                $instance->placeholder = isset($this->field['new_pass_placeholder']) ? $this->field['new_pass_placeholder'] : sprintf(__("New %s", $bkpkFM->name), $this->placeholder);
            }
            $instance->_setInputAttr();
        }
