...
abstract class Base
{
    ...
    {
        $this->inputID = empty($this->field['input_id']) ? "bkpk_field_{$this->field['id']}_{$this->uniqueID}" : $this->field['input_id'];
    }

    protected function _setInputClass()
    {
        $this->addInputClass("bkpk_field_{$this->field['id']}");
        $this->addInputClass('bkpk_input');
        if (! empty($this->field['field_class'])) {
            $this->addInputClass($this->field['field_class']);
        }
        if (! empty($this->field['is_parent'])) {
            $this->addInputClass('bkpk_parent');
        }
    }
    ...
    protected function _setLabelClass()
    {
        ...
        if ($this->titlePosition == 'top') {
            $this->addLabelClass('bkpk_label_top');
        } elseif ($this->titlePosition == 'left') {
            $this->addLabelClass('bkpk_label_left');
        } elseif ($this->titlePosition == 'right') {
            $this->addLabelClass('bkpk_label_right');
        } elseif ($this->titlePosition == 'inline') {
            $this->addLabelClass('bkpk_label_inline');
        } elseif ($this->titlePosition == 'hidden') {
            $this->addLabelClass('bkpk_hidden');
        }
    ...
    {
        $this->descriptionAttr['class'] = ! empty($this->field['description_class']) ? $this->field['description_class'] : 'bkpk_description';
        if ($this->titlePosition == 'left') {
            $this->descriptionAttr['class'] .= ' bkpk_left_margin';
        }
    }
    ...
    {
        $class = [];
        $class[] = 'bkpk_field_container llms-form-field type-text llms-cols-10';
    ...
    protected function _setInputAttr()
    {
        ...
        $this->inputAttr = array(
            'name' => 'bkpk-field-'.$this->inputName.'-'.$this->field['id'],
            ...
            'placeholder' => __($this->placeholder, $bkpkFM->name)
        );
        $this->inputAttr = $bkpkFM->arrayRemoveEmptyValue($this->inputAttr);
    }
    ...
    protected function renderLabel($attr = [])
    {
        ...
        return Html::label(__(! empty($label) ? $label : $this->label, $bkpkFM->name), [
    ...
    protected function renderDescription()
    {
        ...
        if (isset($this->field['description'])) {
            return Html::tag($element, __($this->field['description'], $bkpkFM->name), $this->descriptionAttr);
        }
    }
    ...
    protected function getConditionScript()
    {
        if (! empty($this->field['condition'])) {
            return '<script type="text/json" class="bkpk_condition">' . json_encode($this->field['condition']) . '</script>';
        }
    }
    ...
    public function render()
    {
        ...
        if (! empty($this->javascript)) {
            \BPKPFieldManager\addFooterJs($this->javascript);
        }
