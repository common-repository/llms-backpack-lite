...
class Field
{
    ...
    public function __construct($id, $data = [], $options = [])
    {
        ...
        if (empty($this->data) && ! empty($this->id)) {
			...
			//NOTE: change here for other type fields 
            $fields = $bkpkFM->getData('fields_1');
    ...
    public function render($field = [], $extra = [])
    {
        ...
        $field = apply_filters('llms_bkpk_field_config', $field, $this->id, $formKey, $userID);
        $field = apply_filters('llms_bkpk_field_config_render', $field, $this->id, $formKey, $userID);
        ...
        return apply_filters('llms_bkpk_field_display', $html, $this->id, $formKey, $field, $userID);
    }

    /**
     * Get BPKPFieldManager\Field\Generate for unittests
     *
     * @return NULL|object
     */
    ...
    /*
     * Only used in llms-bkpk-field-value shortcode.
     */
    public function displayValue($metaKey = '')
    {
        ...
        $fieldValue = null;
        $user = $bkpkFM->determineUser();
        
        if (! empty($user)) {
            
            if (isset($this->data['default_value']))
                $fieldValue = $bkpkFM->convertUserContent($user, $this->data['default_value']);
    ...
    /*
     * Only used inside llms-bkpk-field shortcode.
     */
    ...
    private function sanitizeField()
    {
        ...
        if (empty(self::$fieldTypes))
            self::$fieldTypes = $bkpkFM->umFields();
