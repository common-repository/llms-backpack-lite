...
class ValidationRule
{
    ...
    private function setMessage()
    {
        ...
        $messages = Array(
            'required' => $bkpkFM->getMsg('validate_required', '%s'),
            'email' => $bkpkFM->getMsg('validate_email'),
            'equals' => $bkpkFM->getMsg('validate_equals', '%s'),
            'unique' => sprintf(__('%1$s: "%2$s" already taken', $bkpkFM->name), '%s', $this->value)
        );
        
        $default = __('Invalid %s', $bkpkFM->name);
    ...
    private function validate_unique()
    {
        ...
        if (isset($this->options['field_name']))
            return $bkpkFM->isUserFieldAvailable($this->options['field_name'], $this->value, $userID);
    ...
    private function validate_current_password()
    {
        ...
        if (! empty($this->value) && empty($_REQUEST[$fieldName])) {
            $this->message = $bkpkFM->getMsg('validate_current_required', '%s');
            return false;
        }
        ...
        if (! empty($user->user_login)) {
            ...
            if (is_wp_error($user)) {
                $this->message = $bkpkFM->getMsg('validate_current_password');
                return false;
            }
        }
