...
class LibWPSupportsModel
{
    ...
    {
        global $user_ID, $bkpkFramework, $wpdb;
        $unique = true;
        $wpUserTable = $bkpkFramework->wpUserTableFieldsArray();
    ...
    {
        global $bkpkFramework, $pfInstance, $wpdb;
        $errors = new \WP_Error();
        
        // Determine Fields
        $userdata = array();
        $metadata = array();
        $wpField = $bkpkFramework->defaultUserFieldsArray();
        ...
        if (! $userID) {
            if (! empty($userdata['user_email']) && empty($userdata['user_login'])) {
                ...
                if (apply_filters('llms_bkpk_username_without_domain', true)) {
        ...
        if ($metadata) {
            foreach ($metadata as $key => $value) {
                if ($userID)
                    update_llms_bkpk($user_id, $key, $value);
                else
                    add_llms_bkpk($user_id, $key, $value);
            }
        }
