...
class LibRawFunctionsModel
{
     ...
     * @return array
     */
    function toArray($data, $fieldSeparator = ',', $keySeparator = '=', $groupStart = ':', $groupEnd = '|')
    {
        $result = array();
        ...
        foreach ($fields as $field) {
            ...
            if (strpos($field, $groupStart) !== false) {
                $field = explode($groupStart, $field);
                $groupKey = trim($field[0]);
                $field = $field[1];
            }
            ...
            if (! empty($groupKey))
                $result[$groupKey][$fieldKey] = $fieldVal;
            else
